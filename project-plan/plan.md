...
The original deck proposed React.js, TypeScript, Socket.IO, MongoDB, Firebase Auth, and Tailwind CSS. The requested change is **Next.js in place of plain React.js**. The rest of the stack is retained, with a few additions that materially help a real-time product at scale. Each change carries a short rationale so it can be accepted or rejected on its own merits.
...
| Auth | **Firebase Auth** (hosts only) | Kept | Managed multi-provider auth + JWT. Participants stay anonymous (no login), so auth only guards host/admin surfaces. |
...
- **Auth alternatives:** Clerk or Auth.js (NextAuth) integrate more natively with Next.js than Firebase. Firebase is retained per the deck, but Clerk is the smoothest Next.js path if the team is open to switching.
...
```
_id, firebaseUid (unique), name, email, role ('host' | 'admin'),
...
**Deliverables**
- Firebase Auth in `apps/web` (email/password + Google) for hosts; auth state via context.
- NestJS auth guard verifying Firebase ID tokens (Admin SDK) → `req.user`; upsert into `users`.
...
apps/api (NestJS):
- AuthModule: integrate firebase-admin. A FirebaseAuthGuard that reads the Bearer ID token,
  verifies it, and attaches the user. On first verified request, upsert a users document
  (firebaseUid, name, email, role='host').
...
apps/web (Next.js):
- Firebase client SDK auth (email/password + Google). An AuthProvider context + useAuth hook.
  Attach the Firebase ID token to all API calls (TanStack Query + a fetch wrapper).
...
Write unit tests for eventCode generation (uniqueness + charset) and the auth guard.
...
- **Security:** Firebase JWT validation on all host surfaces, RBAC (host/admin), Zod input validation + sanitization on every REST body and socket payload, Helmet headers, HTTPS enforcement, per-event/per-IP rate limiting on `qa:ask`/`activity:respond` via Redis, CORS lockdown. (Passwords are handled by Firebase, so no local bcrypt store; documented as an intentional deviation from the deck.)
...
SECURITY (apps/api):
- Enforce the FirebaseAuthGuard on all host/admin REST + host-only socket actions; add an RBAC
  guard for role='admin' endpoints. Validate every REST body and socket payload with the Zod
  schemas (reject + log on failure). Add Helmet, strict CORS (web origin only), and Redis-backed
  rate limiting on qa:ask and activity:respond (per anonId + per IP). Sanitize user text before
  storage/echo. Document that auth/passwords are delegated to Firebase (no local bcrypt store).
...
- Enterprise SSO (SAML/OIDC) for hosts via the auth layer (or Firebase + SAML / a provider like Clerk/Auth0 if adopted).
...
1) Build a Microsoft Teams app using the Teams JS SDK: a meeting-side panel for participants and a
   ...
   (SAML/OIDC) for host accounts at the auth layer so organizations can log in with their IdP.
   Keep anonymous participant join unchanged.
```
...
talks to the existing REST + Socket.IO API. Implement: host login (Firebase) and the anonymous
participant join flow (code/QR scan via expo-camera, persistent anonId via secure storage), plus the
...
**apps/api**: `PORT`, `MONGODB_URI`, `REDIS_URL`, `FIREBASE_PROJECT_ID`, `FIREBASE_CLIENT_EMAIL`, `FIREBASE_PRIVATE_KEY`, `WEB_ORIGIN`, `RATE_LIMIT_WINDOW`, `RATE_LIMIT_MAX`.
**apps/web**: `NEXT_PUBLIC_API_URL`, `NEXT_PUBLIC_SOCKET_URL`, `NEXT_PUBLIC_FIREBASE_*` (apiKey, authDomain, projectId, appId), `NEXT_PUBLIC_APP_URL`.
...
5. **bcrypt (deck's "secure password hashing") is not implemented locally** — authentication is delegated to Firebase, which manages credential hashing; keeping a parallel local password store would be redundant and less secure. If a self-hosted credential store is later required, bcrypt would be reintroduced in the auth service.
6. **Mobile, Zoom/Teams, and AI features are deferred** to Phases 4, 3, and 5 respectively, per the product direction that the MVP excludes them.
